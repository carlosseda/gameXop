...
exports.create = async (req, res) => {
  try {
    ...
    const data = await SocialNetwork.create(req.body)
...
exports.findAll = async (req, res) => {
  ...
  try {
    const result = await SocialNetwork.find(whereStatement)
      .skip(offset)
      .limit(limit)
      .sort({ createdAt: -1 })
      .lean()
    ...
    const count = await SocialNetwork.countDocuments(whereStatement)
...
exports.findOne = async (req, res) => {
  ...
  try {
    const data = await SocialNetwork.findById(id).lean().exec()
...
exports.update = async (req, res) => {
  ...
  try {
    req.body.images = await req.imageService.resizeImages(req.body.images)
    const data = await SocialNetwork.findByIdAndUpdate(id, req.body, { new: true })
...
exports.delete = async (req, res) => {
  ...
  try {
    const data = await SocialNetwork.findByIdAndUpdate(id, { deletedAt: new Date() })
